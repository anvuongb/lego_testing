...
if __name__ == "__main__":
    ...
    marker_size = 3
    line_width = 4
    model_2brick_cross = LegoModel(
        filepath="./ldr_files/2bricks_cross.ldr",
        color_code_file="color_codes.json",
        save_transformation_history=False,
    )
    model_7brick_rotate = LegoModel(
        filepath="./ldr_files/7bricks_rotate.ldr",
        color_code_file="color_codes.json",
        save_transformation_history=False,
    )
    model_9brick_different = LegoModel(
        filepath="./ldr_files/different_bricks.ldr",
        color_code_file="color_codes.json",
        save_transformation_history=False,
    )
    ...
    model_8brick_rotate = (
        model_7brick_rotate + brick
    )  # the type here is LegoModel + Brick, these types overloading may cause confusion later ...
    ...
    model_combine = (
        model_8brick_rotate + model_2brick_cross + model_9brick_different
    )  # the type here is LegoModel + LegoModel
    ...
    plotly_data_center = model_combine.build_plotly_center(
        opacity, marker_size, line_width
    )
    plotly_data_bricks_center = model_combine.build_plotly_bricks_center(
        opacity, marker_size, line_width
    )
