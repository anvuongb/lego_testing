...
class LegoModel(object):
    def __init__(
        ...
        self.model_height = -1
        self.model_height_in_bricks = (
            -1
        )  # this is only correct if there is no floating and no overlapping bricks
    ...
    def update_sorted_bricks_by_height(self):
        ...
        self.model_height = abs(
            self.bricks[self.sorted_bricks_indices[0]].center_y
            - self.bricks[self.sorted_bricks_indices[-1]].center_y
        )
    ...
    def load_from_ldr(self, filepath):
        ...
        df["block_size"] = df["file_block"].apply(
            lambda x: helpers.decode_file_block(x)
        )
        for _, row in df.iterrows():
            # get default translation matrix from ldr
            tm = helpers.build_translation_matrix(
                row.x,
                row.y,
                row.z,
                row.a,
    ...
    def build_plotly_center(self, opacity=0.5, marker_size=3, line_width=4):
        return [
            self.bricks[0]._build_plotly_data(
                [self.center_x],
                [self.center_y],
                [self.center_z],
                1,
    ...
    def rotate_yaxis(self, angle):
        ...
        for idx, _ in enumerate(self.bricks):
            self.bricks[idx].rotate_yaxis(
                angle, pivot_point=[self.center_x, self.center_y, self.center_z]
            )
        self.recalculate_center()
...
class Brick(object):
    def __init__(
        ...
        self.vertices = helpers.build_vertices(
            0,
            0,
            0,
            self.block_size,
            lego_unit_length=self.unit_length,
            lego_unit_height=self.unit_height,
        )
        self.apply_transformation(
            self.tm, update_tm=False
        )  # don't update when initialization
    ...
    def _build_plotly_data(
        self, x, y, z, opacity, line_color, marker_size=2, line_width=4
    ):
        d = go.Scatter3d(
            x=x,
            y=y,
